      ...
      <div class="dates" ref="dates">
        <div class="dates-bg">
          <div class="week-row" v-for="(week, index) in currentDates" :key="index">
            <div class="day-cell" v-for="(day, index) in week" :key="index" :class="{'today' : day.isToday, 'not-cur-month' : !day.isCurMonth}">
        ...
        <div class="dates-events">
          <div class="events-week" v-for="week in currentDates">
            <div class="events-day" v-for="(day, index) in week" :key="index" :class="{'today' : day.isToday, 'not-cur-month' : !day.isCurMonth}" @click.stop="dayClick(day.date, $event)">
              <p class="day-number">{{day.monthDay}}</p>
              <div class="event-box">
                <event-card :event="event" :date="day.date" :firstDay="firstDay" v-for="(event, index) in day.events"
                            :key="index"
    ...
    methods: {
        ...
        for (let perWeek = 0; perWeek < 6; perWeek++) {
          ...
          for (let perDay = 0; perDay < 7; perDay++) {
            week.push({
              ...
              isToday: monthViewStartDate.isSame(moment(), 'day'), // isSame是否相同：http://momentjs.cn/docs/#/query/is-same/
      ...
      slotEvents(date) {
        ...
        const thisDayEvents = this.events.filter(day => {
          ...
          const end = moment(day.end ? day.end : start)
